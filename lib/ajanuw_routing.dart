...
class AjanuwRoutings {
  ...
  /// 所有动态路由
  List<AjanuwRouting> get dynamicRoutings => routers.values.where((routing) => routing.isDynamic).toList();

  AjanuwRouting findDynamic(String name) {
    return dynamicRoutings.firstWhere((dynamicRouting) => _matchDynamicRoute(name, dynamicRouting));
  }
  ...
  bool _matchDynamicRoute(String routeName, AjanuwRouting dynamicRouting) {
    ...
    final bool equalRouteLength = routeNameSplit.length == dynamicRouteNameSplit.length;
    return equalRouteLength && dynamicRouting.exp!.hasMatch(routeNameSplit.join(pattern));
  }
}

/// A，是arguments参数的类型
class AjanuwRouting<A> {
  final AjanuwRoute<Object> route;
  AjanuwRouting<Object>? routing;
  ...
  Route<T> builder<T extends dynamic>() {
    ...
    if (route.isAnimatedRoute) {
      return PageRouteBuilder<T>(
        settings: _settings,
        transitionDuration:
            route.transitionDurationBuilder != null ? route.transitionDurationBuilder!(routing!) : route.transitionDuration,
        pageBuilder: (context, animation, secondaryAnimation) => _createBuilder(context, route),
  ...
  Widget _createTitle(BuildContext context, AjanuwRoute? route) {
    return Title(
      ...
      child: route.builder!(context, routing!),
    );
  }

  Widget _createBuilder(BuildContext context, AjanuwRoute<Object> route) {
    return route.title != null || route.color != null ? _createTitle(context, route) : route.builder!(context, routing!);
  }
