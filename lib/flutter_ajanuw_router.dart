...
class AjanuwRouter extends AjanuwNavigatorBase {
  ...
  bool _hasCanActivate(AjanuwRouting routing) {
    if (routing.route.canActivate == null || routing.route.canActivate!.isEmpty) return true;
      ...
      if (route.hasChildren) {
        _forRoot(
          route.children!.map((childRoute) {
            return childRoute.canActivate == null ? childRoute.copyWith(canActivate: route.canActivate) : childRoute;
          }),
          _parentPath,
        );
      }
  ...
  Route<T> onGenerateRoute<T>(RouteSettings? settings) {
    AjanuwRouteSettings ajanuwRouteSettings = AjanuwRouteSettings.extend(settings: settings);
    ...
    // push('home'); 相对路径
    // push('../../home'); 相对路径
    if (urlPath.isRelative(routeName) && history.isNotEmpty && history.last != null) {
    ...
    AjanuwRouting routing = routings.find(routeName) ?? routings.find(AjanuwRoute.notFoundRouteName)!;

    //  没有注册[**]路由
    if (routing == null) {
      print('AjanwuRouter Error: Did not find the registered [**] route, return to the blank page');
    ...
    if (routing.isDynamic) {
      routing = routing.copyWith(
        settings: ajanuwRouteSettings.copyWith(paramMap: _snapshot(routeName, routing)),
      );
    }
    ...
    if (routing.isRedirect) {
      return onGenerateRoute(ajanuwRouteSettings.copyWith(name: routing.route.redirectTo));
    }
